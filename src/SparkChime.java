...
class SparkChime extends PApplet {
   ...
   */
  float LOW_BOUNCE = (float)0.5;

  /*
   * The highest ratio of vertical speed retained after a spark bounces.
   */
  float HIGH_BOUNCE = (float)0.8;

  /*
   * The variation in velocity of newly-created sparks.
   */
  float SPRAY_SPREAD = (float)2.0;

  /*
   * Some predefined gravity settings to play with.
   */
  float EARTH_GRAVITY = (float)(1.0 / 16.0);
  float MOON_GRAVITY = (float)(EARTH_GRAVITY / 6.0);
  float JUPITER_GRAVITY = (float)(EARTH_GRAVITY * 2.5);
  ...
  public SparkChime(Tunnel t, int w, int h, String p) {
    width = w;
    height = h;
    tunnel = t;
    position = p;
  ...
  public void draw() {
     ...
     * Convert the prior and current mouse screen coordinates to model coordinates.
       */
      Point3D prior = canvas.toModelCoordinates(ptrackX, ptrackY);
      Point3D current = canvas.toModelCoordinates(trackX, trackY);
     ...
     * Increase the skip count.
      ...
      lastFrameDrawn = now;
      averageElapsedMillis =(float)( .90 * averageElapsedMillis + .10 * elapsedMillis); 
  ...
  float averageElapsedMillis = (float)20.0;
  ...
  public class Canvas3D {
    ...
    public void drawLine(Point3D from, Point3D to, float weight) {
      float stroke = scaleToScreen(weight, to.z);
      if(stroke>0)
        strokeWeight(stroke);
     ...
     * This is a cheat, but it looks fine and is faster than
       * doing it right.
       */
      ellipse(p.x, p.y, screenRadius, (float)(screenRadius * .3));
    }
  }
