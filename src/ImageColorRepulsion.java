...
class ImageColorRepulsion extends PApplet {
  ...
  public ImageColorRepulsion(Tunnel t, int w, int h, String p) {
    width = w;
    height = h;
    tunnel = t;
    position = p;
 ...
 //       generateColors();
      ...
      if ((ptrackX != trackX) || (ptrackY != trackY))
      {
        PMouse.set(trackX, trackY, 0);
      } else
  ...
  class MyPoint
  {
    ...
    final static float FRICTION_AIR = (float).9;//'air' FRICTION_AIR
    final static float FRICTION_POINTS = (float).35;
    ...
    public void process()
    {
      ...
      if (d < MAX_DIST_MOUSE)
      {
        float f =(float).13 * (d - MAX_DIST_MOUSE) / (d);//force
  ...
  public static class CircumCircle
  {
    public static boolean circumCircle(PVector p, Triangle t, PVector circle)
    {
      ...
      if (abs(t.p2.y-t.p1.y) < EPSILON) {
        ...
        mx2 = (float)((t.p2.x + t.p3.x) * .5);
        my2 = (float)((t.p2.y + t.p3.y) * .5);
        circle.x = (float)((t.p2.x + t.p1.x) * .5);
        circle.y = (float)(m2 * (circle.x - mx2) + my2);
      } else if (abs(t.p3.y-t.p2.y) < EPSILON) {
        m1 = - (t.p2.x-t.p1.x) / (t.p2.y-t.p1.y);
        mx1 = (float)((t.p1.x + t.p2.x) * .5);
        my1 = (float)((t.p1.y + t.p2.y) * .5);
        circle.x = (float)((t.p3.x + t.p2.x) *.5);
        circle.y = (float)(m1 * (circle.x - mx1) + my1);
      } else {
        m1 = - (float)((t.p2.x-t.p1.x) / (t.p2.y-t.p1.y));
        m2 = - (float)((t.p3.x-t.p2.x) / (t.p3.y-t.p2.y));
        mx1 = (float)((t.p1.x + t.p2.x) * .5);
        mx2 = (float)((t.p2.x + t.p3.x) * .5);
        my1 = (float)((t.p1.y + t.p2.y) * .5);
        my2 = (float)((t.p2.y + t.p3.y) * .5);
        circle.x = (float)((m1 * mx1 - m2 * mx2 + my2 - my1) / (m1 - m2));
        circle.y = (float)(m1 * (circle.x - mx1) + my1);
      }
  ...
  static class LineIntersector {
    ...
    static final float e = (float)1e-5;
  ...
  class Triangle
  {
    ...
    PVector center() {
      return LineIntersector.simpleIntersect(p1.x, p1.y, (float)((p2.x + p3.x)*.5), (float)((p2.y + p3.y)*.5), p2.x, p2.y, (float)((p3.x + p1.x)*.5),(float)((p3.y + p1.y)*.5));
    }
  }
  ...
  class Triangulator
  {
    ...
    void triangulate(ArrayList<PVector> pxyz, ArrayList<Triangle> triangles)
    {
      ...
      float
        ...
        dmax = dx > dy ? dx : dy, 
        two_dmax = dmax*2, 
        xmid = (float)((xmax+xmin) * .5), 
        ymid = (float)((ymax+ymin) * .5);
