...
class MovieEqualizer extends PApplet {
  ...
  class Visualizer {
    ...
    void drawEQ() { 
      ...
      fill(hVal, 150, 150);//cycles through hue and brightness to expose a greater color palete 
      ...
      w=width/(int)tunnel.getAudioAverage();
      strokeWeight(w/3);//the width of line will be equally spacing
      strokeCap(ROUND);
      int numBars  = (int)tunnel.getAudioAverage();
      int barWidth = width / numBars ;

      for (int i=0; i<tunnel.getAudioAverage(); i++)
      {

        line((i*w)+(w), height, (i*w)+(w), height-tunnel.getAudioAverage()*5);
